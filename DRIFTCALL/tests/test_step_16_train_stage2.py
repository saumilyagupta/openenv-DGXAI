"""Tests for cells/step_16_train_stage2.py.

CUDA + heavy deps mocked: ``unsloth``, ``trl``, ``wandb``, ``peft`` are
injected via ``sys.modules``. Covers:

- Constructor accepts stage param (Stage-2 fixed at 2).
- num_steps default == 200 (DESIGN.md §10.3 Stage-2 row).
- resume_from required (None rejected) — must be a ``pathlib.Path``.
- Language mix exactly 30/30/20/10/10 (en/hinglish/hi/ta/kn).
...
from cells import step_16_train_stage2 as stage2
from cells.step_12_gemma_boot import BF16SlippageError
from cells.step_16_train_stage2 import (
...
def _install_fake_unsloth(
    ...
    *,
    ...
    unsloth_mod.FastModel = FastModel
...
def _install_fake_peft(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    """Install a fake ``peft.PeftModel`` whose ``from_pretrained`` returns a model."""
...
class TestStage2Constants:
    def test_stage_constant_is_2(self) -> None:
        assert STAGE == 2

    def test_default_num_steps_is_200(self) -> None:
        assert DEFAULT_NUM_STEPS == 200

    def test_warmup_ratio_is_zero(self) -> None:
        assert WARMUP_RATIO == 0.0

    def test_default_output_dir(self) -> None:
        assert Path("checkpoints/stage2_final") == DEFAULT_OUTPUT_DIR

    def test_stage_base_seed_is_two_million(self) -> None:
        assert STAGE_BASE_SEED == 2_000_000
...
class TestStage2LanguageMix:
    def test_language_mix_30_30_20_10_10(self) -> None:
...
class TestBuildRunPlan:
    def test_returns_frozen_plan(self) -> None:
        plan = build_run_plan(resume_from=Path("checkpoints/stage1_final"))
    ...
    def test_default_num_steps_200(self) -> None:
        plan = build_run_plan(resume_from=Path("checkpoints/stage1_final"))
        assert plan.num_steps == 200
    ...
    def test_resume_from_must_be_path(self) -> None:
        with pytest.raises(TypeError, match="must be a pathlib.Path"):
            build_run_plan(resume_from=cast("Any", "checkpoints/stage1_final"))
    ...
    def test_default_language_weights_pass_validation(self) -> None:
    ...
    def test_stage_pinned_to_2(self) -> None:
        plan = build_run_plan(resume_from=Path("x"))
        assert plan.stage == 2

    def test_resume_from_preserved(self, tmp_path: Path) -> None:
        plan = build_run_plan(resume_from=tmp_path / "stage1")
        assert plan.resume_from == tmp_path / "stage1"


# ---------------------------------------------------------------------------
# write_local_csv_row + save_checkpoint (parity with stage 1 schema)
# ---------------------------------------------------------------------------
...
class TestLocalCsvCallback:
    def test_header_then_row(self, tmp_path: Path) -> None:
        ...
        write_local_csv_row(csv_path=csv_path, logs={"step": 7, "train/reward_mean": 0.42})
...
# ---------------------------------------------------------------------------
# BF16 assertion + WandB fallback + train()
# ---------------------------------------------------------------------------
...
class TestTrainEntry:
    ...
    def test_train_resumes_from_checkpoint(
        ...
        assert trainer.train_calls[0] == {
            "resume_from_checkpoint": str(tmp_path / "stage1")
        }

    def test_train_attaches_stage1_adapters(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        _full_train_setup(monkeypatch)
        ...
        assert kwargs.args[1] == str(tmp_path / "stage1")
...
class TestModuleSurface:
    def test_train_callable(self) -> None:
        assert callable(stage2.train)

    def test_module_no_pragma_violations(self) -> None:
        text = Path(stage2.__file__).read_text(encoding="utf-8")

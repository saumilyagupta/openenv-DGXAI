...
from typing import Any, List, Optional, Tuple, Union, Dict, Set, Callable
from transformers.models.gemma3n.modeling_gemma3n import (F, math, Callable, Sequence, Optional, torch, nn, init, ACT2FN, Cache, DynamicCache, create_causal_mask, create_sliding_window_causal_mask, BaseModelOutputWithPast, ModelOutput, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, PreTrainedModel, Unpack, TransformersKwargs, maybe_autocast, Gemma3nAudioConfig, Gemma3nTextConfig, Gemma3nVisionConfig, Gemma3nTextScaledWordEmbedding, Gemma3nTextDecoderLayer, Gemma3nPreTrainedModel, Gemma3nTextModel, Gemma3nAudioSSCPConvBlock, Gemma3nAudioConformerLightConv1d, create_causal_mask, create_sliding_window_causal_mask)

@torch.compile(fullgraph = True, dynamic = True, options = torch_compile_options)
def Gemma3nRMSNorm_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
...
class Gemma3nRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return Gemma3nRMSNorm_forward(self, hidden_states=hidden_states)
...
class Gemma3nTextAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return Gemma3nTextAttention_forward(self, hidden_states=hidden_states, position_embeddings=position_embeddings, attention_mask=attention_mask, past_key_values=past_key_values, **kwargs)
...
class Gemma3nTextModel(Gemma3nPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return Gemma3nTextModel_forward(self, input_ids=input_ids, per_layer_inputs=per_layer_inputs, attention_mask=attention_mask, position_ids=position_ids, past_key_values=past_key_values, inputs_embeds=inputs_embeds, use_cache=use_cache, **kwargs)
...
def Gemma3nRotaryEmbedding_forward(self, x, position_ids, layer_type=None):
...
class Gemma3nRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids, layer_type=None):
        return Gemma3nRotaryEmbedding_forward(self, x=x, position_ids=position_ids, layer_type=layer_type)

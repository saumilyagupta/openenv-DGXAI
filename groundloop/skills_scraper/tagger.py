...
# Tokens are runs of alphanumerics, optionally dot-joined (so "next.js" survives).
_TOKEN_RE = re.compile(r"[a-z0-9]+(?:\.[a-z0-9]+)*")


def _tokenize(text: str) -> frozenset[str]:
    """Split lowercased `text` into alphanumeric tokens on non-word boundaries."""
    return frozenset(_TOKEN_RE.findall(text.lower()))


def _matches(keyword: str, tokens: frozenset[str], joined_lower: str) -> bool:
    # Multi-word keywords can't be single tokens; fall back to substring.
    if " " in keyword:
